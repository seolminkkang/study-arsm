...
> **이 문서의 역할** — [[001-cheatsheet]]의 부연이다.
> 회차 중에는 커닝페이퍼를 치고, **"왜 이렇게 하는가"가 필요할 때 여기를 연다.**
> 리허설 준비(체크리스트·시간 재기·화면 구성)도 여기 있다.
>
> | 커닝페이퍼 | 이 문서 |
> |---|---|
> | 시작 전 확인 / 합의 / 오늘 쓰는 ID | 공통 조건 |
> | B-1 ~ B-4 | B급 — EXPLAIN 4개 (같은 이름) |
> | A-1 관찰 / 계산 / 예측 / 실행 | A-1 ①~⑤ — **이름과 순서가 다름.** A-1 머리말의 대응표 참고 |
> | A-2 오프셋 vs 커서 / 인덱스 걸면? | A-2 ⑥ |
> | A-2 ⑦ 통계 미리 집계 | A-2 ⑦ |
> | 원복 규칙 (표) | 각 실험 끝에 흩어져 있음. 표는 커닝페이퍼에만 |
> | 막혔을 때 / 안 다루는 것 | 없음 — 커닝페이퍼에만 |
>
> **한 대로 진행한다.** 이 문서 뒤쪽의 "두 대 구성"은 미채택 참고자료다.
...
### 전환점이 움직인 두 번째 축 — 테이블 물리 상태

위가 "같은 상태에서도 흔들린다"였다면, 이건 "상태가 바뀌면 아예 딴 값이 된다"다.

| 상태 | 테이블 | 페이지 | 전환점 |
|---|---|---|---|
| 2026-08-28 최초 측정 (부분 부풀림) | 미기록 | 미기록 | 15.9~16.2% |
| **B-1 직후** (부풀림 + `movie_id_str` 있음) | 735 MB | 94,019 | **10.5~10.8%** |
| **VACUUM FULL 후** (깨끗) | 약 330~370 MB | 41,917~ | **33.2~36.2%** |
...
- 3단계(4500, 36.2%)에서 두 계획의 `cost` 차이가 1% 안쪽인 것 —
  "조금씩 나빠지는 게 아니라 어느 순간 뚝 떨어진다"를 보여주는 지점
- 옵티마이저 `rows` 추정치와 실제 `rows` 값의 차이

**#미실험:** Index Scan → Bitmap Heap Scan 전환점(0.0003%~0.1% 사이)은 안 좁혔다.
필요하면 당일 전에 추가로 찾아본다.
...
이번엔 추정이 틀려도 스캔 방식은 안 바뀌었지만, B-2의 36% 경계 근처에서는
...
> **[[001-cheatsheet]]와의 대응** — 이름과 순서가 다르다. 커닝페이퍼에서
> 막혔을 때 이 표로 찾아온다.
>
> | 커닝페이퍼 | 이 문서 |
> |---|---|
> | A-1 관찰 | **③ 실측** |
> | A-1 계산 | **① 느린 쿼리 만들기** + **② 이론 최대 TPS 계산** |
> | A-1 예측 | (이 문서에 없음 — 진행표 1:10–1:25 참고) |
> | A-1 실행 · 풀 2→50 | **④ 커넥션 풀 크기 바꾸기** |
> | A-1 실행 · 타임아웃 증폭 | **⑤ 재시도 증폭** |
>
> **순서가 반대인 게 의도다.** 이 문서는 ①②(계산) → ③(실측) 순으로 적혀 있지만
> 회차에서는 **실측을 먼저 보고 계산을 나중에** 한다. 계산을 먼저 하면
> 답을 알고 실험하게 되어 "역시 그렇네"로 끝난다.
...
> ## ⚠ 한 대로 진행하기로 정했다 (2026-08-30)
>
> 아래 두 대 구성은 **채택하지 않았다.** 참고용으로만 남긴다.
> 실제 진행은 [[001-cheatsheet]]를 따르며, 거기에는 한 대 기준만 있다.
>
> **한 대로 충분한 이유:** 이 실험들은 전부 같은 기계에서 조건만 바꾸는
> 상대 비교다. 그리고 부하가 0.5~0.7 TPS 수준이라 k6가 CPU를 거의 안 뺏는다.
> 대신 **"절대값 말고 상대값만 본다"를 시작할 때 합의하고 들어간다.**
