...
> **슬라이드 버전은 `001-overview.html`이다.** 회차 시작 5분간 팀원과 같이 본다.
...
> 이 절은 `001-overview.html` 슬라이드의 원문이다. 슬라이드로 띄워 말하고,
> "왜 이렇게 만들었나"가 필요할 때 여기를 읽는다.
...
Moha Cinema 프로젝트에서 **DB 스키마와 영화 데이터만** 가져왔다. 앱은 새로 짰다.

원본 백엔드에는 OAuth, JWT, Security 필터, WebFlux, MyBatis, 추천 서비스가
얽혀 있다. 부하를 걸면 응답 시간에 그게 전부 섞여서 **"쿼리가 느린 건지
다른 게 느린 건지" 구분이 안 된다.** 병목을 하나만 만들어두려고 파일 5개짜리
앱을 새로 만들었다.
...
k6도 앱도 DB도 **전부 노트북 한 대**에서 돈다.
그래서 절대값은 못 믿는다 — 부하를 올리면 k6가 CPU를 먹어서 앱이 느려지는데,
이건 앱이 느려진 게 아니라 노트북이 느려진 것이다.
...
평점 500만 건은 원본에 583건뿐이라 만들어 넣었다. **그런데 일부러 치우치게 만들었다.**
...
> "조회 대상에 따라 DB가 다르게 동작한다"는 걸 아예 볼 수 없다.
> B-2(선택도)와 B-3(커버링 인덱스)이 통째로 죽는다.
...
당일에는 [[001-cheatsheet]]의 같은 카드를 본다. 여기에는 왜 그 줄만 보는지를 적는다.

### EXPLAIN — 이 세 줄만 본다

| | 어디 | 뭘 보나 | 왜 |
|---|---|---|---|
| ① | **맨 윗줄** | `Index Only Scan` / `Index Scan` / `Bitmap Heap Scan` / `Seq Scan` 중 뭔가 | **B급 4개의 전부가 이 한 줄이다.** 조건을 바꿨을 때 이 이름이 바뀌는지가 관측 대상이다 |
| ② | `Rows Removed by Filter`<br>`Heap Fetches` / `Heap Blocks` | 읽고 **버린 양**, 테이블을 **몇 번 찾아갔나** | 시간이 같아도 여기가 다를 수 있다. B-1이 정확히 그 경우다 — 529ms vs 527ms인데 한쪽만 500만 행을 읽고 버렸다 |
| ③ | `Execution Time` | 실제 걸린 시간. **참고만** | 같은 쿼리도 OS 페이지 캐시 상태에 따라 배로 달라진다. B-3 배율이 1,788배 → 128배 → 21배로 요동친 게 이 때문이다 |

**나머지(`cost`, `width`, `Recheck Cond`, `Planning Time`, `JIT`)는 오늘 안 본다.**
`cost`는 옵티마이저의 추정치지 실측이 아니다. 궁금하면 물어보되 판단 근거로는 안 쓴다.

### Grafana — 네 패널, 언제 뭘 보나

**B급(EXPLAIN)에서는 이 화면을 안 본다. A급부터 본다.**
부하가 없으면 3·4번은 바닥에 평평하다 — 그게 정상이다.

| | 패널 | 볼 때 | 왜 |
|---|---|---|---|
| 1 | TPS (초당 처리한 건수) | 부하를 계단식으로 올리는데 **이게 안 오르기 시작하는 지점** | 그 지점이 포화점이다. 부하를 더 줘도 처리량이 안 늘면 거기가 한계다 |
| 2 | p95 (느린 쪽 5%) | 1번은 평평한데 **이것만 치솟는 순간** | 평균은 안 본다. 일부만 느린 것을 평균이 뭉갠다 |
| 3 | `pending` (줄 선 수) | **0에서 올라가는 순간.** 같은 패널의 `active`가 `max`와 같으면 풀이 꽉 찬 것 | `pending`은 DB 커넥션을 못 얻어 대기 중인 스레드 수다. 0보다 크면 대기가 생긴 것 |
| 4 | DB CPU | **190% 위에 붙어 있나** (`cpus: 2`라 200%가 상한) | 여기 붙어 있으면 병목은 앱이 아니라 DB다. 풀을 키워도 소용없다 |

> **A급의 하이라이트:** 1번은 안 오르는데 2·3번은 치솟고 4번은 190%에 붙어 있다.
> 그때 던질 질문 — **"커넥션을 더 주면 빨라질까?"**
>
> 여기서 산술 모델(`풀 크기 / 쿼리 시간 = 최대 TPS`)을 꺼낸다.
> 이 식대로면 풀을 키우면 처리량이 늘어야 한다. 4번 패널이 그 예측을 반박한다.
...
질문이 나오면 "이번엔 안 다뤄"라고 답하고 [[progress]]에 적어둔다.
