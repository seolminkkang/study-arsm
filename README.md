...
1회차 당일(2026-08-30) 실측입니다. 노트북 한 대에서 k6·앱·DB가 CPU를 나눠 쓰므로
절대값이 아니라 조건을 바꿨을 때의 방향만 봅니다.
측정 원본은 [vault/raw/](vault/raw/2026-08-30_exp-001_session_results.txt)에 있습니다.

- **커넥션 풀을 10에서 50으로 5배 늘렸는데 처리량이 그대로였다** — 1.2616 → 1.2567 TPS.
  실패율만 97.94% → 99.70%로 늘었다. 산술 모델의 예측은 8.6 TPS였는데 실측이 1.26 TPS다.
  DB 컨테이너가 `cpus: 2`라 상한을 정한 건 풀 크기가 아니라 DB CPU였다.
  ([exp-002](vault/experiments/exp-002-pool-size.md))
- **인덱스를 걸었더니 오프셋 조회가 4배 느려졌다** — `updated_at` 인덱스 하나로
  오프셋은 2.97초 → 12.99초, 커서는 0.29초 → 0.014초. 같은 인덱스에 두 방식이
  반대로 움직였다. **책에 없는 내용이다.**
  ([exp-003](vault/experiments/exp-003-offset-vs-cursor.md))
- **커버링 인덱스가 깨지는 순간 테이블 블록 9,310개를 읽으러 갔다** — 조회 칼럼에
  `updated_at` 하나를 더하자 `Index Only Scan`(`Heap Fetches: 0`, 0.77ms)이
  `Bitmap Heap Scan`(16.4ms)으로 바뀌었다.
  ([exp-001](vault/experiments/exp-001-covering-index.md))

회차 기록: [2026-08-30 1회차](vault/sessions/2026-08-30-1회차.md)
