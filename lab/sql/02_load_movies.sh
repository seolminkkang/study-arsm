...
#   ./02_load_movies.sh
#
# ⚠ app/ 폴더는 절대 읽지 않는다. 실제 사용자 이메일과 비밀번호 해시가 들어 있다.
#   실습에 필요한 건 movies/ 의 3개 파일뿐이다.
#   (app/moha_user_account_*.sql 146명, 98_demo_login.sql 평문 비밀번호)
#
...
#
# 읽는 파일은 아래 세 개뿐이다. 와일드카드도 디렉터리 순회도 쓰지 않는다 —
# app/ 폴더에 실제 사용자 이메일과 비밀번호 해시가 있어서, 파일명을 명시해
# 그 폴더가 실수로 읽힐 수 없게 해둔 것이다. 여기에 파일을 추가하지 마라.
# ─────────────────────────────────────────────────────────────
